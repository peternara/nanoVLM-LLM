...
#    상세)
#    → 각 토큰 위치와 차원별 주파수를 곱해 : 이것이 sin,cos에 들어갈 입력 𝜃
...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # ii) RoPE 주파수 공식: freq_i = 1/(base^(2i/dim))
        #     → freq_i > 이걸 왜 구하는가? cos, sin 에 angle을 구하기 위한 절차!   
        #         → 즉, 다음 수식(𝜃 구하기)에 적용하기 위한 수식 
        #             → angle(𝜃) = pos×freq_i
        ...
        #
        #                → 다시 말해, interleave(교차) 방식 
        #                    → 각 θ값을 
        #                        → 짝수 차원(0,2,4,6...): cos(θ₀, θ₁, θ₂, θ₃) 
        #                        → 홀수 차원(1,3,5,7...): sin(θ₀, θ₁, θ₂, θ₃) 
        #                    → 이렇게 교차해서(번갈아) 곱하는 것
        #     → 이제 RoPE 주파수 공식 대입해보자!
        #         → d: 한 헤드의 차원수 (self.dim)        
        #         → base: 주로 10000, 100000 등
        #         → 예) d=8, dim=10000 이면 tensor([1, 4.6416, 2.1544, 0.1]) 
        #             → 0: 1/1 = 1.0
